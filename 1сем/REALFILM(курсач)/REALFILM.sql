...
ALTER TABLE MOVIE ALTER COLUMN Photo NVARCHAR(MAX);
...
WHERE ID_MOVIE = (SELECT ID FROM MOVIE WHERE TITLE = 'first1');


DELETE FROM ORDERS
WHERE ID_SCHEDULE IN (
    SELECT ID FROM SCHEDULE
    WHERE ID_MOVIE = (SELECT ID FROM MOVIE WHERE TITLE = 'Øðýê')
);


DELETE FROM ORDERS
WHERE ID = 'ac93020f-ab5e-4280-b366-8396c620aa35';