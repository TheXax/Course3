...
void CPyramid::Draw1(CDC& dc, CMatrix& PView, CRect& RW)			// с удалением ребер
{
	...
	// Вывод значений PView (углов наблюдения)
	char buf[50] = "";
...
void CPyramid::Draw11(CDC& dc, CMatrix& PView, CRect& RW) // с удалением ребер
{
	CMatrix XV = CreateViewCoord(PView(0), PView(1), PView(2)); //Создает матрицу преобразования из мировой системы координат в видовую систему
	CMatrix ViewVert = XV * this->Vertices; //координаты вершин пирамиды в видовые координаты
	CRectD RectView;
	GetRect(this->Vertices, RectView);
	CMatrix MW = SpaceToWindow(RectView, RW); //из мировой системы в оконные

	CPoint MasVert[6], A1B1C1[3], ABC[3];
	CMatrix V(3);
	V(2) = 1;

	//преобразуем координаты вершин пирамиды в оконные координаты
	for (int i = 0; i < 6; i++)
	{
		V(0) = ViewVert(0, i);
		V(1) = ViewVert(1, i);
