...
string GetStatusText(CA::Status status) { //для получения текстового представления состояния
	switch (status) {
...
int _tmain(int argc, _TCHAR* argv[])
{
	...
	try
	{
		s = InitCA((char*)"192.168.67.209", (char*)"\\\\IVAN\\Lab2folder\\output.txt", 1);  //\\\\IVAN\\Lab2folder\\output.txt - Ваня   \\\\DESKTOP-HPV9KK7\Users\RoBiN\
		// айпи координатора
		...
		while (true)
		{
			...
			bool close = false;
			std::string line;
			std::ofstream ss("\\\\IVAN\\Lab2folder\\output.txt", std::ios::app); // сетевая папка для записи
			std::ifstream s2("\\\\IVAN\\Lab2folder\\output.txt", std::ios::app); // сетевая папка для чтения
			...
			if (k == 1) {
				...
				for (int i = 0; i < 5; i++)
				{
					...
					if (ss.is_open())
					{
						ss << "Client Veronika: " << s.clientnumber << " " << asctime(timeinfo) << std::endl;
					}
					Sleep(1000);
				}
				LeaveCA(s);
...
CA InitCA(char ipaddr[15], char resurce[40], int number)
{
	CA result;
