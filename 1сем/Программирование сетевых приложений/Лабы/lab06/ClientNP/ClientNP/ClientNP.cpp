...
#define PIPEG TEXT("\\\\DESKTOP-9OIST0B\\pipe\\Box")//указывает на доступный канал по сети
#define PIPEL TEXT("\\\\.\\pipe\\Box")//локальный канал
...
int main()
{
	try {
		...
		if ((cH = CreateFile(PIPEL, //имя канала
			...
			NULL, //атрибуты безопасности не заданы
			OPEN_EXISTING, //открытие существующего канала
			NULL, //доп флаги и атрибуты не нужны
		...
		int count;
		cout << "Введите кол-во сообщений,которые хотите отправить: ";
		cin >> count;

		for (int i = 1; i <= count; i++) {
			//запись данных в канал с помощью WriteFile
			if (!WriteFile(cH, &buffer, strlen(buffer - 1), &dwWrite, NULL)) {
				throw SetPipeError("WriteFile:", GetLastError());
			}
			//чтение ответа с помощью ReadFile
