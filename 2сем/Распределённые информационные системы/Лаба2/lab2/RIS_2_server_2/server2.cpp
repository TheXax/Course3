...
class Link //для хранения информации о каждом клиенте, который пытается получить доступ к определенному файлу
{
public:
    char* FileName;
    char command; //е - вход, l - выход
    bool UsingFile;
    SOCKADDR_IN Adr;
    HANDLE WaitEvent;
    HANDLE Thread;
...
DWORD WINAPI Dispath(LPVOID lp)
{
    ...
    queue<SOCKADDR_IN>* q = new queue<SOCKADDR_IN>(); //очередь клиентов
    int index = (int)lp;
    while (true)
    {
