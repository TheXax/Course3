...
namespace lab2
{
    public class Entropy
    {
        ...
        {
            var symbolAppearances = new Dictionary<char, int>(); //словарь с символами и их кол-вом
        ...
        {
            var symbolAppearances = GetSymbolAppearances(str); //для получения частот символов
            var entropy = 0d;
            foreach (var item in symbolAppearances)
            {
                var P = (double)item.Value / str.Length;
                entropy -= P * Math.Log2(P); //подсчёт энтропии
            }
