...
//конструктор по умолчанию|матрица 1х1
CMatrix::CMatrix()
{
 n_rows=1;
 n_cols=1;
...
//Оператор, позволяющий получить доступ к элементам матрицы по индексам
...
//Оператор для доступа к элементу вектора
...
//Возвращает новую матрицу, элементы которой являются отрицательными значениями исходной матрицы.
...
//Проверяет, совпадают ли размеры матриц, и возвращает новую матрицу, содержащую суммы соответствующих элементов
...
//Оператор вычитания матриц, аналогично операции сложения
CMatrix CMatrix::operator-(CMatrix& M)
// Оператор M1-M2
{	
	int bb=(n_rows==M.rows())&&(n_cols==M.cols());
...
//Проверяет размеры и вычисляет произведение
CMatrix CMatrix::operator*(CMatrix& M)
...
//Оператор присваивания, копирующий значения из матрицы M. Проверяет размеры и предотвращает самоприсваивание
...
//создает новую матрицу, копируя значения из существующей
...
//Метод, изменяющий размер матрицы, уничтожая старые данные
...
//Метод, изменяющий размер матрицы, сохраняя данные, если это возможно
...
//Метод для изменения размера матрицы до NewRow с одной колонкой
...
//Метод для изменения размера матрицы с одной колонкой, сохраняя данные
