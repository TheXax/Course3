...
namespace REALFILM
{
    ...
    {
        private bool styleCheck = false; //тема
        ...
        public MainFrameUser()
        {
            ...
            listOfFilms = context.MovieRepo.GetAllMovies();//все фильмы из репозитория
            mainFilmsListView.ItemsSource = listOfFilms;//устанавливает источник данных для списка фильмов
        }
        ...
        { 
            ...
            var selectedFilm = (sender as Button)?.DataContext as Movie;//данные о выбранном фильме
            var tickets = context.ScheduleRepo.GetMovieTickets(selectedFilm);// доступные билеты
        ...
        {
            ComboBoxItem selectedBoxItem = filterGenre.SelectedValue as ComboBoxItem;//выбранный жанр
            string selectedGenre = selectedBoxItem.Content.ToString();
            var regex = new Regex(@"(\w)*" + selectedGenre + @"(\w*)", RegexOptions.IgnoreCase);
            var newList = new List<EFCore.Entities.Movie>();//новый список фильмов по жанру
        ...
        private void Grid_KeyUp(object sender, KeyEventArgs e)
        {
            if (e.Key == Key.Enter)
            {
                TextBox txtBox = e.Source as TextBox;//элемент управления, вызвавший событие
