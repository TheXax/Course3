...
struct GETSINCHRO
{
	DWORD64 mTime; //время отклика
	__int64 mcorr; //общая коррекция
	__int64 mCcOs; //разница между клиенто и сервером
};

struct SETSINCHRO
{
	__int64 corrTime; //коррекция от сервера
};
	...
	while (true) {
		...
		try
		{
			...
			for (int i = 0; i < 10; i++)
			{
				...
				//обработка полученного времени и коррекции
