...
#получаем значение координатора из файла
...
while True:
    try:
        ...
        proxy.settimeout(2)
        proxy.sendto(msg, (coordinator, PORT)) #отправляет оригинальное сообщение координатору
        response, _ = proxy.recvfrom(1024)

        sock.sendto(response, addr) #Отправляет ответ обратно клиенту
