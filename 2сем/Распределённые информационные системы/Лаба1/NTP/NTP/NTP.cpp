...
int main()
{
	...
	long double avg_cc = 0;

	long double avg_oc_cc = 0;
	...
	try
	{
		...
		int recv_buf_size = 1024 * 1024; // 1 МБ
		if (setsockopt(serverSocket, SOL_SOCKET, SO_RCVBUF, (char*)& recv_buf_size, sizeof(recv_buf_size)) < 0) {
			std::cerr << "Ошибка при установке размера буфера для приема" << std::endl;
		}
		...
		while (true)
		{
			...
			if (gs.mTime != 0) {
				avg_i++;
				avg = ((avg * (avg_i - 1)) + ss.corrTime) / avg_i;

				cout << inet_ntoa(clientParameters.sin_addr) << "\ncorrectionTime:" << ss.corrTime << "  avgCorrection:" << avg << "\nclientTime:" << gs.mTime << "\n\n";
			}

			if (gs.mcorr != 0) {
				client_i+=10;
				avg_cc = ((avg_cc * (client_i - 10)) + gs.mcorr) / client_i;
				avg_oc_cc = ((avg_oc_cc * (client_i - 10)) + gs.mCcOs) / client_i;

				cout << inet_ntoa(clientParameters.sin_addr) << " end\n";
				cout << "avg correction from all clients: " << avg_cc << endl;
				cout << "avg CC - OStime from all clients: " << avg_oc_cc << endl;;
			}
