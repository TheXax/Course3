...
int main()
{
	try {
		...
		DWORD dwWrite;//кол-во байт, которые запишем
		DWORD bytes;//байты, прочитанные из канала
		...
		//Пытается выполнить вызов к именованному каналу. одновременное 
		if (!CallNamedPipe(PIPEL,
			buffer,
			sizeof(buffer),
			outbuffer,
			MAX_SIZE_OF_BUFFER,
