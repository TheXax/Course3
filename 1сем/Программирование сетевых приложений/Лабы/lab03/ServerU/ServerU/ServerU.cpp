...
int main() {
	...
	try {
		...
		SOCKADDR_IN serv;				   // параметры сокета
		...
		SOCKADDR_IN client; 
		memset(&client, 0, sizeof(client)); // обнулить память (для предотвращения случайных данных в полях структуры)
		int lc = sizeof(client);
		char ibuf[50]; // буфер для хранения входящих смс
		...
		while (true) {
			...
			while (true) {
				...
				//recvfrom: функция, которая блокирует выполнение программы до тех пор, пока не будет получено сообщение
				...
				// Случайный выбор, чтобы решить, терять сообщение или нет
				/*if (rand() % 2 == 0) { // 50% шанс потерять сообщение
					cout << "Сообщение потеряно (не отправлено обратно)." << endl;
					continue; // Пропустить отправку ответа
				}*/
