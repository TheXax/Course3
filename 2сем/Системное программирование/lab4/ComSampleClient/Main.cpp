...
HRESULT Execute(_In_ const IID &rclsid, _In_ DWORD dwCoInit, _In_ DWORD dwClsContext)
{
    ...
    if (SUCCEEDED(hr))
    {
        ...
        if (SUCCEEDED(hr))
        {
            ...
            if (SUCCEEDED(hr))
            {
                wprintf(L"%s. Client calling from %s. COM Server running %s.\n", 
                        pwszWhoAmI,
                        (dwCoInit == COINIT_MULTITHREADED) ? L"MTA" : L"STA", //для определения, используется ли многопоточный или однонаправленный подход
        ...
        //ISVA
        //Создание COM-объект интерфейса ISVA
        ISVA* pSva;
        hr = CoCreateInstance(rclsid,
            NULL,
            dwClsContext,
        ...
        if (SUCCEEDED(hr))
        {
            int number = 123456;  
            int result = 0;
            hr = pSva->getThirdDigitFromEnd(number, &result);
            if (SUCCEEDED(hr))
            {
                wprintf(L"ISVA::getThirdDigitFromEnd(%d) returned %d\n", number, result);
            }
            else
            {
                wprintf(L"ISVA::getThirdDigitFromEnd failed: 0x%08X\n", hr);
            }
            pSva->Release();
        }
        else
        {
            wprintf(L"QueryInterface for ISVA failed: 0x%08X\n", hr);
        }
