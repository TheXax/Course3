	...
	try
	{
		...
		while (true)
		{
			...
			if (ibuf.status == ibuf.ENTER || ibuf.status == ibuf.WAIT)
			{
				...
				{
					cout << "Enter in Critical section " << inet_ntoa(clnt.sin_addr) << endl;
			...
			{
				cout << "Leave from Critical section " << inet_ntoa(clnt.sin_addr) << endl;
				map.erase(map.find(ibuf.resurce));
				ibuf.status = ibuf.LEAVE;
			}
			else if (ibuf.status == ibuf.INIT)
			{
				cout << "Init Critical Section " << inet_ntoa(clnt.sin_addr) << endl;
			}
