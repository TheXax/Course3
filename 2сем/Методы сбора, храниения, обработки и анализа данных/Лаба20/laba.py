...
#переименовываем классы в «blue» и «red» для удобства
y_named = np.array(["blue", "red"])[y]

#можно вызвать train_test_split с любым количеством массивов, все будут разбиты одинаковым образом
...
#Значение показывает, насколько сильно модель уверена в том, что точка данных принадлежит «положительному» классу
#Положительное значение указывает на предпочтение в пользу позиционного класса, а отрицательное значение – на предпочтение в пользу «отрицательного»
...
plt.show()
#Граница принятия решений (слева) и решающая функция (справа) модели градиентного бустинга, построенной на двумерном синтетическом наборе данных
