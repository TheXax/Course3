...
//новые значения для координат прямоугольника
...
//размеры и координаты объектов в системе координат
CBlade::CBlade()            
{
	double rS = 30;   //радиус лопасти    
	double RoE = 10 * rS;	 //радиус окружности вращения
	double d = RoE;		    
	RS.SetRectD(-d, d, d, -d);					// Область системы в мировых координатах
	RW.SetRect(0, 0, 690, 640);					// Область в окне
	MainPoint.SetRect(-rS, rS, rS, -rS);		// Начальная точка
...
//обновляет координаты различных точек в зависимости от углов поворота и времени
void CBlade::SetNewCoords()
{

	double RoV = (WayRotation.right - WayRotation.left) / 2; //радиус вращения
	double ff = (fiFT / 90.0) * pi;
	//Вычисляет и устанавливает новые координаты для передней лопасти
	double x = RoV * cos(ff);
	double y = RoV * sin(ff);
	FTCoords(0) = x;
	FTCoords(1) = y;
	...
	//обновление угла
	fiFT += wPoint * dt; //wPoint - угловая скорость
	CMatrix P = CreateRotate2D(fiFT);	//создает 2D-матрицу поворота на заданный угол
	FTCoords = P * FTCoords;		//применяем матрицу поворота к текущим координатам
	...
	fiST += wPoint * dt;
	P = CreateRotate2D(fiST); //создает 2D-матрицу поворота на заданный угол
	STCoords = P * STCoords; //Новые координаты лопасти обновляются путем умножения текущих координат на матрицу поворота
	...
	double horizontalFF = (fiHFT / 90.0) * pi; //угол вращения
	double horizontalX = RoV * cos(horizontalFF); //новые координаты по углу
	double horizontalY = RoV * sin(horizontalFF);
	FTHCoords(0) = horizontalX;
	FTHCoords(1) = horizontalY;
	FTHCoords(2) = 1;

	fiHFT += wPoint * dt; //обновление угла по угловой скорости
...
void CBlade::Draw(CDC& dc)
{
	...
	dc.MoveTo(0, 0); //начало координат
...
//Аффинное преобразование — это обобщение линейного преобразования, которое сохраняет коллинеарность (размеры и пропорции) и параллельность линий,
//но может изменять размеры, поворачивать и смещать объекты в пространстве. Это преобразование описывается матрицей и вектором смещения.