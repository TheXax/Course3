...
db.on('DELETE', (req, res) => {
    ...
    if (typeof url.parse(req.url, true).query.id != "undefined")//извлекает пар-тр id
    {
...
http.createServer((request, response) => {
    if (url.parse(request.url).pathname === '/' + route)
        db.emit(request.method, request, response); //для вызыва соответствующего события
    else if (url.parse(request.url).pathname === '/')
    {
        ...
        response.end('<html><body><h1>Error! Visit http://localhost:5000/' + route + '</h1></body></html>');
