...
class CComServerTest : public IComTest, public ISVA {
public:
  // IUnknown
...
/*Проверяет, поддерживает ли объект запрашиваемый интерфейс. Если да, возвращает указатель на него и увеличивает счетчик ссылок*/
      ...
      *ppv = nullptr;
      ...
      if (__uuidof(IComTest) == riid) {
          *ppv = static_cast<IComTest*>(this);
          hr = S_OK;
      } else if (__uuidof(ISVA) == riid) { //ISVA
          *ppv = static_cast<ISVA*>(this);
          hr = S_OK;
