...
struct HDFS {  //Структура, представляющая файловую систему
    std::fstream file;
};

int clnum = 1; //номер клиента
...
HDFS OpenDFSFile(const char* FileName) {
    if (section.status == CA::INIT || section.status == CA::ENTER) {
        ...
        InitCA("192.168.67.209", "\\\\IVAN\\Lab2folder\\output.txt", clnum);  //192.168.67.115
    }
...
int main() {
    ...
    HDFS hdfs = OpenDFSFile("\\\\IVAN\\Lab2folder\\output.txt");  //\\\\IVAN\\Lab2folder\\output.txt - Ваня   \\\\DESKTOP-HPV9KK7\Users\RoBiN\Desktop\Lab2\LabFol - Дима  192.168.67.115 - Дима
    ...
    while (true) {
        ...
        if (choice == 1) {
            ...
            for (int i = 0; i < 10; ++i) {
                ...
                string data = "Client Veronika | Line " + to_string(i + 1) +
