...
function Fact(k, func)
{
    ...
    //используется для добавления функции в очередь. Функция выполнится как можно скорее, но не блокируя основной поток выполнения
}
...
http.createServer((request, response) => {
    ...
    else if (url.parse(request.url).pathname === '/')
    {
        ...
        response.end(rc);
}).listen(5000, () => console.log('Server running at http://localhost:5000/fact'));